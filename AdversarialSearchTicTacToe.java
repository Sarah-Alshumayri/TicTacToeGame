/*
Description:
This code implements the Minimax algorithm for an adversarial search in the game of
Tic-Tac-Toe. The minMaxDecision function takes a current game state and returns the
best move for the X player. The algorithm evaluates all possible moves by recursively
applying the minValue and maxValue functions. The isTerminal function checks if the game
is over by examining the state and returns true if there is a winner or all spots are taken.
The utilityOf function assigns a utility value (+1 for X win, -1 for O win, and 0 for a draw)
to terminal states. The successorsOf function generates all possible successor states for
a given state.
